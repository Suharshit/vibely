...
export function usePhotos(eventId: string) {
  ...
  const uploadPhoto = useCallback(
    async (
      ...
      try {
        ...
        // WHY createUploadTask instead of uploadAsync?
        // In this Expo version, uploadAsync options no longer expose
        // onUploadProgress. createUploadTask provides a typed progress
        // callback while still uploading binary content.
        const uploadTask = FileSystem.createUploadTask(
          ...
          (progress) => {
            ...
            const pct = Math.round((progress.totalBytesSent / total) * 85);
        ...
        if (!uploadResult) {
          throw new Error("Storage upload failed: no response");
        }
        ...
        const completeRes = await fetch(
          `${API_BASE}/api/photos/${photo_id}/complete`,
          {
            method: "POST",
            headers: authHeaders,
          }
  ...
  const savePhoto = useCallback(async (photoId: string) => {
    ...
    setPhotos((prev) =>
      prev.map((p) => (p.id === photoId ? { ...p, saved_by_me: true } : p))
    );
  }, []);
  ...
  const unsavePhoto = useCallback(async (photoId: string) => {
    ...
    setPhotos((prev) =>
      prev.map((p) => (p.id === photoId ? { ...p, saved_by_me: false } : p))
    );
  }, []);
