...
export default function EventDetailScreen() {
  ...
  const {
    photos, isLoading: photosLoading, hasMore,
    uploads, uploadPhoto, loadMore,
  ...
  const fetchEvent = useCallback(async () => {
    try {
      ...
      const me = eventData?.event_members?.find((m: any) => m.user?.id === user.id);
      setUserRole(me?.role ?? '');
      setEvent(eventData);
    } catch (err: any) {
      ...
      {!expired && (
        <View style={styles.card}>
          ...
          <View style={styles.qrWrapper}>
            <QRCode value={inviteUrl} size={160} color="#111827" backgroundColor="#ffffff" />
          </View>
