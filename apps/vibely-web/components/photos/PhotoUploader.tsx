...
import { validateImageFile, MAX_FILE_SIZE_BYTES, ALLOWED_MIME_TYPES } from '@shared/utils/storage';
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3">
          {/* Upload icon */}
          <div className={`p-3 rounded-full transition-colors ${isDragging ? 'bg-violet-100' : 'bg-white border border-gray-100'}`}>
            <svg className={`w-6 h-6 ${isDragging ? 'text-violet-600' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Drag & drop or click to browse · JPEG, PNG, WebP, HEIC · Max 10MB each
            </p>
          </div>
        </div>
      </div>
        ...
        {isError ? (
          ...
          <div className="mt-1.5">
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {upload.status === 'completing' ? 'Finishing…' :
               isDone ? 'Uploaded!' :
               `${upload.progress}%`}
            </p>
          </div>
