...
async function getAuthAndMembership(eventId: string) {
  ...
  if (authError || !user) return { error: 'Unauthorized', status: 401, supabase, user: null, membership: null };
...
export async function GET(_req: Request, { params }: RouteParams) {
  ...
  const { error, status, supabase, user, membership } = await getAuthAndMembership(id);
...
export async function PATCH(request: Request, { params }: RouteParams) {
  const { id } = await params;
  const { error, status, supabase, membership } = await getAuthAndMembership(id);
