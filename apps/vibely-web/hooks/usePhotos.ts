...
interface UsePhotosReturn {
  ...
  uploadFiles: (files: File[], eventId: string, guestToken?: string) => Promise<void>;
  uploads: UploadItem[];
...
export function usePhotos(eventId: string): UsePhotosReturn {
  ...
  const fetchPage = useCallback(async (page = 1) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/events/${eventId}/gallery?page=${page}&limit=20`);
      if (!res.ok) {
  ...
  const uploadFiles = useCallback(async (
    ...
    await Promise.all(
      newUploads.map(item => uploadSingleFile(item.file, eventId, item.id, guestToken))
    );
  }, [uploadSingleFile]);
  ...
  const savePhoto = useCallback(async (id: string) => {
    ...
    setPhotos(prev => prev.map(p => p.id === id ? { ...p, saved_by_me: true } : p));
  }, []);

  const unsavePhoto = useCallback(async (id: string) => {
    await fetch(`/api/photos/${id}/save`, { method: 'DELETE' });
    setPhotos(prev => prev.map(p => p.id === id ? { ...p, saved_by_me: false } : p));
  }, []);

  return {
    photos, pagination, isLoading, error,
    fetchPage, uploadFiles, uploads,
    deletePhoto, savePhoto, unsavePhoto,
  };
}